...
exports.encodeFeature = encodeFeature;
exports.decodeFeature = decodeFeature;
exports.encodeGeometry = encodeGeometry;
exports.decodeGeometry = decodeGeometry;
function encodeFeatureCollection(message) {
    let bb = popByteBuffer();
    _encodeFeatureCollection(message, bb);
    return toUint8Array(bb);
}
function _encodeFeatureCollection(message, bb) {
...
function decodeFeatureCollection(binary) {
    return _decodeFeatureCollection(wrapByteBuffer(binary));
}
function _decodeFeatureCollection(bb) {
    let message = {};
...
function encodeFeature(message) {
    let bb = popByteBuffer();
...
function _encodeFeature(message, bb) {
    ...
    let $geometry = message.geometry;
    if ($geometry !== undefined) {
        writeVarint32(bb, 18);
        let nested = popByteBuffer();
        _encodeGeometry($geometry, nested);
        writeVarint32(bb, nested.limit);
        writeByteBuffer(bb, nested);
        pushByteBuffer(nested);
    }
    // optional map<string, string> properties = 3;
...
function _decodeFeature(bb) {
    ...
    end_of_message: while (!isAtEnd(bb)) {
        ...
        switch (tag >>> 3) {
            ...
            case 2: {
                let limit = pushTemporaryLength(bb);
                message.geometry = _decodeGeometry(bb);
                bb.limit = limit;
                break;
            }
            // optional map<string, string> properties = 3;
...
function encodeGeometry(message) {
    let bb = popByteBuffer();
    _encodeGeometry(message, bb);
    return toUint8Array(bb);
}
function _encodeGeometry(message, bb) {
    // optional string type = 1;
    let $type = message.type;
    if ($type !== undefined) {
        writeVarint32(bb, 10);
    ...
    let array$coordinates = message.coordinates;
    if (array$coordinates !== undefined) {
        let packed = popByteBuffer();
        for (let value of array$coordinates) {
...
function decodeGeometry(binary) {
    return _decodeGeometry(wrapByteBuffer(binary));
}
function _decodeGeometry(bb) {
    let message = {};
    end_of_message: while (!isAtEnd(bb)) {
        let tag = readVarint32(bb);
        switch (tag >>> 3) {
            ...
            // repeated double coordinates = 2;
            case 2: {
                let values = message.coordinates || (message.coordinates = []);
                if ((tag & 7) === 2) {
                    let outerLimit = pushTemporaryLength(bb);
                    while (!isAtEnd(bb)) {
